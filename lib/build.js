...
async function compileOnUnix (nodeVersion, targetArch) {
  ...
  if(mem <= 1024 * 1024 * 1024 && threads > 2) {
    threads = 2;
  } else if (mem <= 4* 1024 * 1024 * 1024 && threads > 4) {
    threads = 4
  }


  const args = ['--fully-static', '--without-intl'];
  ...
  const promise = spawn(make, ['-j'+threads], { cwd: nodePath }); //HAS to be 2 since the pi3 will run oom with more threads
