...
export async function main () {
  const patchesJson = {
    'v16.0.0-proposal': [ 'node.v16.0.0.cpp.patch' ]
  };
  const targetArchs = [];

  //Because we're not cross-compiling, we're doing this
  switch(process.arch) {
    case "arm":
      targetArchs.push("armv7");
      break;
    case "arm64":
      targetArchs.push("arm64");
      break;
    case "x64":
      targetArchs.push("x64");
      break;
  }
  ...
  for (const nodeVersion in patchesJson) {
    for (const targetArch of targetArchs) {
      ...
      try {
        console.log("This would now upload the local file", local, remote);
      } catch (error) {
